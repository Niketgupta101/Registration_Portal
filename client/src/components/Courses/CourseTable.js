      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead onClick={() => handleShowClick()}>
          <TableRow className="table-row">
            ...
            <TableCell align="right" style={{ display: displayBody }}>Number of Unplaced Students</TableCell>
        ...
        <TableBody style={{ display: displayBody }}>
          {rows.map((row) => (
            ...
            >
              ...
              <TableCell align="right"><a href={"https://"+row.courseLink}>See Course Details</a></TableCell>
...
function CourseTable2(props) {
  ...
  useEffect(() => {
    ...
    let filterData = [];
    for (var i = 0; i < temp.length - 1; i++) {
      filterData.push(createData2(temp[i][0], temp[i][1], temp[i][2],temp[i][3]));
    }
    if (temp.length > 0) {
